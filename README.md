...
| 模块 | 文章数 | 内容概览 |
| --- | ---: | --- |
| 深度学习 | 15 | 系统学习神经网络、优化算法、向量化、卷积网络、循环网络、Transformer 等基础与模型结构。 |
| RAG | 9 | 围绕检索增强生成的学习笔记、实现思路和相关工程模块记录。 |
| Agent | 4 | 记录 Agent 基础模块、系统提示词、交互流程和工具调用相关内容。 |
| CNN | 5 | 卷积神经网络、深度卷积模型、目标检测、人脸识别和风格迁移等专题。 |
| RNN | 3 | 循环神经网络、序列模型和注意力机制相关学习记录。 |
| Codex | 2 | Codex 安装、桌面端使用和工具链体验记录。 |
| Claude Code | 1 | Claude Code 命令行版安装与使用记录。 |
| 学习杂谈 | 3 | 建站记录、阶段性随笔和学习过程中的零散思考。 |
...
文章图片统一使用 `/images/...` 形式引用，由 Hexo 根据 `root` 配置生成 `/hexo-blog/images/...`，以适配 GitHub Pages 项目站点路径。
